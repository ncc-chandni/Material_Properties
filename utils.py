...
###########################################################
'''
The MidpointNormalize class is a custom normalization class for use in data visualization, particularly with colormaps. It normalizes data values to a range between 0 and 1, with a specific midpoint that is central in the normalization range. This is useful when you want to emphasize differences around a central value (e.g., zero) and handle data that is symmetrically distributed around that value.
...
Inherits from Normalize class, which is a standard normalization class from Matplotlib's colors module.
Initialization (__init__ method):

Parameters:
vmin: Minimum data value that the normalization maps to 0.
vmax: Maximum data value that the normalization maps to 1.
midpoint: The central value that is normalized to 0.5. This is the point of reference where the data is centered.
clip: If True, data values outside the range [vmin, vmax] are clipped to the interval.
Initialization:
Calls the parent class Normalize's __init__ method with vmin, vmax, and clip.
Stores midpoint in the instance.
Normalization (__call__ method):

Parameters:
value: The data values to normalize.
clip: Not used here but passed to maintain consistency with the parent class.
Process:
Defines x as an array of the boundaries [vmin, midpoint, vmax] and y as the corresponding normalized values [0, 0.5, 1].
Uses np.interp to perform linear interpolation between x and y to map the data values to the normalized range [0, 1].
Returns a masked array to handle any values that fall outside the normalization range.
'''
...
# Create a midpoint normalization instance
norm = MidpointNormalize(vmin=1, vmax=9, midpoint=5)

# Create a colormap
cmap = plt.get_cmap('coolwarm')

# Plot the data with the custom normalization
fig, ax = plt.subplots()
sc = ax.scatter(data, data, c=data, cmap=cmap, norm=norm)
...
def plot_2d_grid_search(grid, midpoint=0.7, vmin=0, vmax=1):
    ...
    scores = grid.cv_results_['mean_test_score'].reshape(len(param1), len(param2))
